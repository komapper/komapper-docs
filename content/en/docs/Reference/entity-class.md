...
The `length` property specifies the maximum length for string and binary column types (e.g., VARCHAR, CHAR, BINARY).

The `precision` property specifies the maximum number of digits for numeric column types (e.g., DECIMAL, NUMERIC).

The `scale` property specifies the number of digits to the right of the decimal point for numeric column types (e.g., DECIMAL, NUMERIC).
...
This will generate DDL with appropriate column constraints based on the database dialect.
...
If the `insertable` property is set to `false`, the column is excluded from `INSERT` statements generated by
[`insert()`]({{< relref "Query/QueryDsl/insert" >}}) DSLs.
This is useful for columns with database-generated default values, computed columns, or auto-incrementing fields.
